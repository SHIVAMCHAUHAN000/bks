...
export const api = {
  ...
  importLeads: (csv: FormDataEntryValue | null, url: FormDataEntryValue | null) => request<{ imported: number; rows: number }>('/api/import', { method: 'POST', body: JSON.stringify({ CSV: csv, URL: url }) }),
  sendCampaign: (payload: Record<string, FormDataEntryValue>) => request<{ sent: number }>('/api/automation/send', { method: 'POST', body: JSON.stringify(payload) }),
